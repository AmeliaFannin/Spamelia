...
def get_ham_sample
  ...
  Dir.foreach("#{Dir.pwd}/easy_ham/") do |name|
    next if name == '.' or name == '..' or name == 'cmds'

    File.open "#{Dir.pwd}/easy_ham/#{name}", "r" do |f|
      f.each_line { |l| input << l }
    end
    @ham_size += 1
  end
...
def count_frequency
  ...
  get_ham_sample.each do |s|
    # removes non UTF-8 characters
    s.force_encoding("iso-8859-1").gsub!(/\W/, ' ')

    # s.gsub!(/^(((ht|f)tp(s?))\://)?((([a-zA-Z0-9_\-]{2,}\.)+[a-zA-Z]{2,})|((?:(?:25[0-5]|2[0-4]\d|[01]\d\d|\d?\d)(?(\.?\d)\.)){4}))(:[a-zA-Z0-9]+)?(/[a-zA-Z0-9\-\._\?\,\'/\\\+&amp;%\$#\=~]*)?$)/, '')

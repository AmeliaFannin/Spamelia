...
class Spammer
  ...
  def calc_spamminess(word1)
    # word = word.downcase
    ham = count_frequency(@ham[1])
    ham_size = @ham[0]

    spam = count_frequency(@spam[1])
    spam_size = @spam[0]

    total_sample = (ham_size + spam_size).to_f
    spam_word = spam.fetch(word1, 0)
    total_word = ham.fetch(word1, 0) + spam_word

    puts "#{word1} occurs in Spam: #{spam[word1]} of #{spam_size}"
    puts "#{word1} occurs in Ham: #{ham[word1]} of #{ham_size}"

    # puts "#{word2} occurs in Spam: #{spam[word2]} of #{spam_size}"
    # puts "#{word2} occurs in Ham: #{ham[word2]} of #{ham_size}"

    num = (spam_word / total_sample) 
    denom = (total_word / total_sample)
    percent_spammy = (num / denom) 

    puts percent_spammy * 100
  end

  # calc_spamminess('penis', 'free')
end
